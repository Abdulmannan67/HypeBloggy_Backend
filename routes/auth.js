...
const Notes = require("../models/Blogs");
   ...
   const errors = validationResult(req);
    ...
    try {
      ...
      if (user) {
        return res
          .status(400)
          .json({ error: "A user with this email already exists",message:"A user with this email not exists" });
      }
  ...
  try {
    let user = await User.findOne({ email: req.body.email });
      if (!user) {
        return res
          .status(400)
          .json({ error: "A user with this email not exists",message:"A user with this email not exists" });
      }
      const otp = Math.floor(Math.random()*10000)
      const mailOptions = {
        from: process.env.EMAIL,
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(400)
          .json({  error: "Please login correct credentials",message:"A user with this email not exists" });
      }
